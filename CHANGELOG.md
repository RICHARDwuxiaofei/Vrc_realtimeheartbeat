...
- 手机扫码固定为竖屏，并移除 ZXing 默认扫描激光线，避免预览横置和中央竖线。
- 手机与手表统一为 `1/5/10 秒`三档；任意一端选择后通过 Wear Data Layer 双向同步，运行中的手表会立即切换采集/传输策略。
- 手表与手机都新增匹配的 `production` / `diagnostic` 包名组合，正式版和诊断版可以同时安装，同时保持各自的 Data Layer 通道可用。
- Windows UI 重构为与手机端一致的深色卡片风格，重新组织实时心率、链路状态、曲线、来源、工具和日志；曲线卡始终可见，并支持整页鼠标滚轮滚动。
- Galaxy Watch Classic 的正式版和诊断版页面新增旋转表圈滚动支持。
