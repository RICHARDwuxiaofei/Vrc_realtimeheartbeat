...
_EXACT: Final[dict[str, tuple[str, str]]] = {
    ...
    "发现新版本": ("New version available", "新しいバージョンがあります"),
    "打开 Release": ("Open Release", "Releaseを開く"),
    "忽略此版本": ("Ignore this version", "このバージョンを無視"),
    "关闭": ("Close", "閉じる"),
    "忽略更新": ("Ignore update", "更新を無視"),
    "更新是可选的，可以关闭此窗口；忽略后本版本不再提醒。": (
        "The update is optional. Close this window, or ignore this version to stop reminders for it.",
        "更新は任意です。このウィンドウを閉じるか、このバージョンを無視すると再通知されません。",
    ),
    "手机 UDP 端口": ("Phone UDP port", "スマートフォンUDPポート"),
    "VRChat OSC 端口": ("VRChat OSC port", "VRChat OSCポート"),
    "手机/手表发送频率": ("Phone/watch relay interval", "スマートフォン/Watch送信間隔"),
    "1 秒（实时）": ("1 s (real-time)", "1秒（リアルタイム）"),
    "5 秒（省电）": ("5 s (power saver)", "5秒（省電力）"),
    "10 秒（超省电）": ("10 s (maximum power saver)", "10秒（超省電力）"),
    "电脑修改后通过 ACK 同步到手机，再由手机同步到手表": (
        "PC changes are sent in the ACK, then the phone syncs the watch",
        "PCの変更はACKでスマートフォンへ送信され、Watchへ同期されます",
    ),
    "发送频率": ("Relay interval", "送信間隔"),
...
_PHRASES: Final[dict[str, tuple[str, str]]] = {
    ...
    "发送频率已切换为每 ": ("Relay interval changed to every ", "送信間隔を"),
    " 秒；下一份手机数据的 ACK 将同步手机和手表": (
        " s; the next phone-data ACK will sync the phone and watch",
        "秒に変更。次のスマートフォンデータのACKでスマートフォンとWatchへ同期します",
    ),
    ...
    "已忽略版本 " : ("Ignored version ", "無視したバージョン "),
    "已是最新版 " : ("Up to date ", "最新版 "),
    "当前版本：" : ("Current version: ", "現在のバージョン："),
    "最新版本：" : ("Latest version: ", "最新バージョン："),
