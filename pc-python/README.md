...
- 发现比当前版本新的 GitHub Release 时显示非强制更新弹窗；可关闭，或忽略该版本后不再提醒。
...
- 手机中转模式可在“手机/手表发送频率”选择 `1 秒（实时）`、`5 秒（省电）`或 `10 秒（超省电）`。
  电脑会把选择放进下一份 `pc_ack`，手机收到后更新手机转发节拍，并沿现有控制通道同步手表；电脑直连小米手环模式不需要此设置。
- 配对二维码优先使用 Windows 网卡表中的已启用物理以太网/Wi-Fi，并在其中优先选择有 IPv4 默认网关的接口，因此 UU/VPN 等虚拟地址不会抢在真实局域网地址前面。

电脑通过每个 UDP 回执中的 `diagnosticMode` 标记控制手机，也通过 `relayIntervalSeconds` 和
`relayIntervalUpdatedEpochMillis` 同步发送频率。手机只在设置时间戳更新时应用远端值，并把最终
值沿原有控制通道同步给手表。普通模式的手表包不包含原始 BPM、精度、电量、屏幕状态和发送模式；
诊断模式才附加这些字段以及手机网络、VPN、接收时间等信息。
