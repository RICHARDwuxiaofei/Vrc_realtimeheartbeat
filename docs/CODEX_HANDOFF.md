...
| VRChat OSC 输出模块 | 部分完成 / 待验证 | EXE 能发送 `HR_Tens`、`HR_Ones`、`HRValid`、本地节拍 `HRPulse` 及旧参数兼容包；已验证 OSC UDP 包实际抵达本机监听器，未验证 VRChat 或 Avatar 实际响应。 |
...
| `/avatar/parameters/HR_Tens` | OSC Int；两位数 BPM 的十位，BPM 超过 99 时按 99 显示 |
| `/avatar/parameters/HR_Ones` | OSC Int；两位数 BPM 的个位 |
...
- 当前两位数显示对超过 99 BPM 的数位输出钳制为 99；完整 BPM 仍通过旧版 `HeartRate` 参数发送。
...
6. **稳定输出 VRChat OSC。** 用真实传感器数据验证 `HR_Tens`、`HR_Ones`、`HRValid`、`HRPulse` 并记录 VRChat 实收；默认 5 秒数值更新和 PC 本地节拍已经分离，不再重复提高采样率。
...
- Python 版实现 UDP 回执、严格数据验证、动态超时、两位数参数、兼容旧参数和 PC 本地 `HRPulse`。诊断包只显示链路成功并返回 ACK，不写入 Avatar 参数。
- Python 本地测试当前为 35 项，覆盖协议、非法输入、OSC 编码、数字拆分、超时、心跳脉冲、运行时开关和真实 UDP 回环。
