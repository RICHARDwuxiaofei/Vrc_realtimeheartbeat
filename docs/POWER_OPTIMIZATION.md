...
2026-07-25 完整复核又补了两处随运行时间或异常状态增长的外围开销：

1. CSV 曲线改为从文件尾部反向读取，抵达所选时间窗边界即停止。此前虽然没有内存历史，但每秒仍会从文件头扫描整次会话，长时间诊断会逐步增加电脑 CPU 和磁盘读取。
2. 正式手表版对相同 warning 事件做 60 秒限频。传感器连续返回非法值等异常情况下，不再每个回调都写 Logcat 和私有文件；测试版不做此限频，仍保留完整诊断证据。

手机待发结构仍保持有界：连续心率只留最新一份，手动诊断另有一个独立槽并优先发送。因此电脑离线既不会堆积历史心率，也不会让用户的一键诊断被下一份心率覆盖。
