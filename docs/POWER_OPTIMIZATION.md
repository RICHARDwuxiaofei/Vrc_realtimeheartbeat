...
电脑、手机和手表新增默认关闭的联动诊断模式。电脑通过 UDP ACK 下发状态，手机只在开关变化时用一条 Data Layer 控制消息通知手表。普通模式的热路径不再发送或保存原始 BPM、传感器精度、手表电量、屏幕状态、手表模式、手机接收时间、手机网络和 VPN 等扩展字段，也不保留电脑端历史曲线或逐包日志；诊断模式才附加这些字段，并把电脑样本追加写入内部 CSV。

曲线默认读取 CSV 最近 1 分钟，可选择 1–10 分钟。写入句柄每行 flush，曲线使用独立只读句柄，因此不需要在内存中长期缓存历史样本。用户导出仍是手动操作。该模式降低的是协议字节数、日志、历史缓存和普通模式的应用侧开销，不会改变 ExerciseClient 连续 PPG 本身的主要耗电。
