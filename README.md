...
电脑“连接与工具”里的“手机/手表发送频率”可选 `1 秒（实时）`、`5 秒（省电）`
或 `10 秒（超省电）`。电脑修改后会写入下一份 UDP 回执，手机收到后更新自己的转发节拍，
再沿现有手机 → 手表控制通道同步手表；手表端链路无需改动。日常建议使用 `5 秒`，长时间挂着
VRChat 可选 `10 秒`。

电脑每次启动都会在后台检查 GitHub 最新 Release。发现比当前版本更新的 Release 时会弹出非强制
提示；可以关闭窗口稍后再看，也可以点击“忽略此版本”，之后该 Release 不再重复提醒。

配对二维码优先使用 Windows 当前已启用的物理以太网/Wi-Fi 网卡，并在其中优先选择有 IPv4
默认网关的接口；像 UU 这类虚拟适配器不会再排在真实以太网（例如 `192.168.100.139`）之前。

手表与手机通过 Wear OS Data Layer 通信；手机与电脑需要位于可互相访问的局域网。酒店 Wi-Fi 可能禁止设备间通信，即使能上网也不代表 UDP 可以互通。

手表正式版检测到正在充电时会停止心率传输服务；如果手表脱下后连续 5 分钟没有有效心率样本，
也会自动结束本次服务，避免空转耗电。这里是正常停止 Exercise 会话和前台服务，不是强制杀 Android 进程。

手机通知栏会显示当前收到的心率，并最多每 5 秒刷新一次；首次使用 Android 13 或更高版本时需允许
“通知”权限。
